...
bool CloudSpeechClient::savemp3(long file_size)
{
  ...
  if (!file)
  {
    Serial.println("open file error! "  + String( textfile));
    return false;
  }
...
//解析这个字串
//{"code": 0, "message": "ok", "resp": "\u611f\u53f9\u5565\u5440\uff1f\u53d7\u5230\u4ec0\u4e48\u5fc3\u7075\u51b2\u51fb\u4e86\uff1f",
//"audio": "http://0.0.0.0:5000/audio/ae436964295ff8de4b5b2d240727ce9d.mp3"}
String CloudSpeechClient::PrintJson_wukong_ret(String line)
{
  const char* p_code;
  String code_msg = "";
#ifdef SHOW_DEBUG
  Serial.println(">" + line);
#endif
  if (line.indexOf("resp") > 0)
  {
    StaticJsonBuffer<1024> jsonBuffer;
    JsonObject& root = jsonBuffer.parseObject(line);

    p_code =  root["message"];
    code_msg = String(p_code);
    Serial.println("message:" + code_msg);

    p_code =  root["resp"];
    code_msg = String(p_code);
    Serial.println("resp:" + code_msg);

    p_code =  root["audio"];
    code_msg = String(p_code);
    Serial.println("audio:" + code_msg);

    p_code =  root["code"];
    code_msg = String(p_code);
    Serial.println("code:" + code_msg);
  }
  return (code_msg);

}

String CloudSpeechClient::posturl_2020(String host, int port, String url, String body)
{

  // Serial.println(String("host:") + host + " url:" + url+ " body:"+body);
  ...
  String  HttpHeader = String("POST ") + url + " HTTP/1.1\r\n" +
                       "Content-Type: application/x-www-form-urlencoded\r\n" +
                       "Host: " + host + ":" + String(port) + "\r\n" +
                       "Content-Length: " + String(body.length()) +  "\r\n" +
                       "Connection: keep-alive\r\n\r\n" ;
#ifdef SHOW_DEBUG
  Serial.print("HttpHeader:" + HttpHeader);
#endif
  client.print(HttpHeader);
